 ...
 * moves.c
 * by Rob Upcraft
 * Updated to OpenMPI 5.0.x by Noah Harris
...
Move* bestMove(char* board, int color, Move* prevMove, int depth, int evaluator)
{
	...
	if (legalMoves == NULL)
	{
		...
		for (i = 1, j = 0; i < procCap - 1; i++, j++)
		{
			...
			MPI_Send(&color, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
			MPI_Send(&depth, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
			MPI_Send(&evaluator, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
			MPI_Send(board, 64, MPI_CHAR, i, 0, MPI_COMM_WORLD);
			sendMoveSet(i, sub);
			sendMove(i, prevMove);
			free(sub);
		}
...
void sendMoveSet(int receiver, MoveSet* set)
{
	...
	for (i = 0; i < size; i++)
	{
		MPI_Send(&set->moves[i]->pieceId, 1, MPI_CHAR, receiver, 0, MPI_COMM_WORLD);
		MPI_Send(&set->moves[i]->startRow, 1, MPI_CHAR, receiver, 0, MPI_COMM_WORLD);
		MPI_Send(&set->moves[i]->startCol, 1, MPI_CHAR, receiver, 0, MPI_COMM_WORLD);
		MPI_Send(&set->moves[i]->endRow, 1, MPI_CHAR, receiver, 0, MPI_COMM_WORLD);
		MPI_Send(&set->moves[i]->endCol, 1, MPI_CHAR, receiver, 0, MPI_COMM_WORLD);
		MPI_Send(&set->moves[i]->boardScore, 1, MPI_INT, receiver, 0, MPI_COMM_WORLD);
	}
}
...
MoveSet* receiveMoveSet()
{
	...
	int i;
	int size;
	MPI_Recv(&size, 1, MPI_INT, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	...
	{
		Move* m = (Move*) malloc(sizeof(Move));
		MPI_Recv(&m->pieceId, 1, MPI_CHAR, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(&m->startRow, 1, MPI_CHAR, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(&m->startCol, 1, MPI_CHAR, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(&m->endRow, 1, MPI_CHAR, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(&m->endCol, 1, MPI_CHAR, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(&m->boardScore, 1, MPI_INT, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		set->moves[i] = m;
	}
...
void sendMoveDef(MoveDef* best)
{
	...
	MPI_Send(&bestIndex, 1, MPI_INT, HEAD, 0, MPI_COMM_WORLD);
	MPI_Send(&bestValue, 1, MPI_INT, HEAD, 0, MPI_COMM_WORLD);
}
...
MoveDef* receiveMoveDef(int sender)
{
	int bestIndex;
	int bestValue;
	MPI_Recv(&bestIndex, 1, MPI_INT, sender, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	MPI_Recv(&bestValue, 1, MPI_INT, sender, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	MoveDef* best = (MoveDef*) malloc(sizeof(MoveDef));
...
{
	MPI_Send(&move->pieceId, 1, MPI_CHAR, receiver, 0, MPI_COMM_WORLD);
	MPI_Send(&move->startRow, 1, MPI_CHAR, receiver, 0, MPI_COMM_WORLD);
	MPI_Send(&move->startCol, 1, MPI_CHAR, receiver, 0, MPI_COMM_WORLD);
	MPI_Send(&move->endRow, 1, MPI_CHAR, receiver, 0, MPI_COMM_WORLD);
	MPI_Send(&move->endCol, 1, MPI_CHAR, receiver, 0, MPI_COMM_WORLD);
	MPI_Send(&move->boardScore, 1, MPI_INT, receiver, 0, MPI_COMM_WORLD);
}
