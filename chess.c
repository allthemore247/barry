...
{	
	...
	if (&proc == HEAD)
	{
		...
		while (TRUE)
		{
			...
			MPI_Recv(&color, 1, MPI_INT, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			
			//Terminate program if color is -1
			if (color == -1)
				break;
			
			MPI_Recv(&depth, 1, MPI_INT, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			MPI_Recv(&evaluator, 1, MPI_INT, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			MPI_Recv(&board, 64, MPI_CHAR, HEAD, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
