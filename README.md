# barry and why its better:

Okay, not really, they're the exact same programs really. However, when attempting to use the [original garry](https://github.com/upcrob/garry), it quickly failed on our cluster of PI's due to changes in the way OpenMPI handled its bindings in version 5.0.9. So, this version of garry really just updates the function calls with OpenMPI; no changes to program logic were actually made.

This file, and the associated changes to moves.c and chess.c made
by Noah Harris at Liberty University.
